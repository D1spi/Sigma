...
export const checkToken = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  if (!token) {
    ...
    next(new ApplicationError('You must be logged in to access this resource.', httpStatus.UNAUTHORIZED));
    return;
  }
  try {
    ...
    next(new ApplicationError('Invalid token.', httpStatus.UNAUTHORIZED));
  }
};
